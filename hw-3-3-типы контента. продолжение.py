...
def get_updates_contact():
    ...
    while True:
        ...
        if update_id!=update_id_new:
            message=r['result'][-1]
            file_id=message['message']['contact']['phone_number']
            user_id=message['message']['from']['id']

            requests.get(f'{BASE_URL}{TOKEN}/sendContact?chat_id={user_id}&contact={file_id}')
    ...
    if isinstance(dic,dict): 
        for key,value in dic.items():
            ...
            if key == 'video':
                ...
                print('есть тел контакт:', get_updates_contact())
            else:
                print('---нет ничего---')
